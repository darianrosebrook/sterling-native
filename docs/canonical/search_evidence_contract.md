...
| `search_tape.stap` | Yes | Binary hot-loop event log (SearchTapeV1) with chain-hash integrity | Always (since M4) |
...
**Artifact count**: 6 (UniformScorer) or 7 (TableScorer).
...
### Metadata binding checks (Steps 12–13)
...
13. **Scorer binding** (if either scorer artifact or graph metadata scorer_digest exists): Bidirectional — both must be present or both absent. Content hash must match. Mismatch → `MetadataBindingScorerMismatch` / `MetadataBindingScorerMissing`.

### Scorer coherence checks (Steps 14–15)

14. **Scorer artifact binding**: If report has `scorer_digest`, `scorer.json` must exist and its content hash must match. Mismatch → `ScorerDigestMismatch`. Artifact missing → `ScorerArtifactMissing`.
...
### Tape verification (Step 16, when tape present)

16a. **Tape presence** (Cert only): If profile is Cert and tape absent → `TapeMissing`. If profile is Base and tape absent → pass (skip remaining tape checks).

16b. **Tape digest binding**: Report `tape_digest` must be present and must match `search_tape.stap` content hash. Missing → `ReportFieldMissing`. Mismatch → `TapeDigestMismatch`.

16c. **Tape parse + chain hash**: Parse tape via `read_tape()`. This internally verifies the chain hash across all records. Parse failure or chain hash mismatch → `TapeParseFailed`.

16d. **Header binding against authoritative artifacts** (fail-closed — all fields required when tape present):
...
16e. **Schema version**: Tape header `schema_version` must be `"search_tape.v1"`. Missing or wrong → `TapeParseFailed`.

16f. **Tape→graph equivalence** (Cert only): Render tape to `SearchGraphV1` via `render_graph()`, serialize to canonical JSON bytes, compare byte-for-byte to `search_graph.json` content. Mismatch → `TapeGraphEquivalenceMismatch`.
...
- All cross-artifact binding digests match (report ↔ graph ↔ policy ↔ scorer).
...
Base does **not** guarantee tape presence. Pre-M4 bundles without tape pass Base verification.
...
Cert is the promotion-eligible profile. A bundle that passes Cert verification provides a complete, tamper-evident evidence chain from search inputs to search outputs.
