...
Status: v0.3-draft  
Primary proving rigs: Rig C, Rig D, Rig F, Rig J, Rig L, Rig M  
Last updated: 2026-01-31
## 1. Problem shape

real-world planning is rarely single scalar cost. You need Pareto handling (time vs risk vs resource burn vs disruption) and a way to surface trade-offs.
...
- ObjectiveVector: {time, risk, resource_burn, disruption, ...}
- PreferenceModel: weights or Pareto policy
- Planner returns either scalarized optimum with declared weights or Pareto frontier subset
## 3. What must be proven

- Trade-offs are explicit and auditable; weight changes move choices predictably
